...
ptr<string> Utils::carray2Hex(const uint8_t *_carray, size_t _len) {
    CHECK_ARGUMENT(_carray);
    ...
    for (size_t j = 0; j < _len; j++) {
        hex->at(j * 2) = hexval[((_carray[j] >> 4) & 0xF)];
        hex->at((j * 2) + 1) = hexval[(_carray[j]) & 0x0F];
    }
