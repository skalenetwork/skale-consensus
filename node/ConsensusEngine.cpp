...
void ConsensusEngine::parseFullConfigAndCreateNode(
    const string& configFileContents, const string& _gethURL ) {
    try {
...
ptr< Node > ConsensusEngine::readNodeTestConfigFileAndCreateNode( const string path,
    ...
    try {
        ...
        auto node = JSONFactory::createNodeFromTestJsonFile( sgxServerUrl,
            nodeFileNamePath.string(), _nodeIDs, this, _useSGX, _sgxSSLKeyFileFullPath,
            _sgxSSLCertFileFullPath, _ecdsaKeyName, _ecdsaPublicKeys, _blsKeyName, _blsPublicKeys,
...
void ConsensusEngine::parseTestConfigsAndCreateAllNodes(
    const fs_path& dirname, bool _useBlockIDFromConsensus ) {
    try {
        ...
        for ( uint64_t j = 0; j < dirNames.size(); j++ ) {
            ...
            readNodeTestConfigFileAndCreateNode( dirNames.at( j ), nodeIDs, isSGXEnabled,
...
void ConsensusEngine::startAll( ptr< vector< uint8_t > > _startingFromSnapshotWithThisAsLastBlock)  {
...
ConsensusEngine::ConsensusEngine( ConsensusExtFace& _extFace, uint64_t _lastCommittedBlockID,
    ...
    cout << "Constructing consensus engine: "
         << ""
            "Last block in skaled: "
         << ( uint64_t ) _lastCommittedBlockID << ' '
         << "Last block in skaled timestamp: " << ( uint64_t ) _lastCommittedBlockTimeStamp << ' '
         << "Last block in skaled human readable timestamp: "
         << std::asctime( std::gmtime( &lastCommitedBlockTimestamp ) )
         << "\n Total storage limit for consensus: " << _totalStorageLimitBytes << endl;
