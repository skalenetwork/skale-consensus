...
void OracleRequestSpec::checkEthApi( const string& _ethApi ) {
    ORACLE_CHECK_STATE3( _ethApi == string( "eth_call" ), "Eth Method is not supported:" + _ethApi,
        ORACLE_ETH_METHOD_NOT_SUPPORTED );
}
...
void OracleRequestSpec::checkURI( const string& _uri ) {
    ...
    ORACLE_CHECK_STATE3(
        _uri.size() <= ORACLE_MAX_URI_SIZE, "Uri too long:" + _uri, ORACLE_URI_TOO_LONG );
    ...
    if ( _uri != "eth://" && !testMode ) {
        ...
        ORACLE_CHECK_STATE3(
            result.userName_.empty(), "Non empty username", ORACLE_USERNAME_IN_URI );
        ORACLE_CHECK_STATE3(
            result.password_.empty(), "Non empty password", ORACLE_PASSWORD_IN_URI );
...
OracleRequestSpec::OracleRequestSpec( const string& _spec ) : spec( _spec ) {
    ...
    d.Parse( spec.data() );
    ORACLE_CHECK_STATE3(
        !d.HasParseError(), "Unparsable Oracle spec:" + _spec, ORACLE_UNPARSABLE_SPEC );


    // first check elements required for all calls

    ORACLE_CHECK_STATE3(
        d.HasMember( "cid" ), "No chainid in Oracle spec:" + _spec, ORACLE_NO_CHAIN_ID_IN_SPEC );
    ORACLE_CHECK_STATE3( d["cid"].IsUint64(), "ChainId in Oracle spec is not uint64_t" + _spec,
        ORACLE_NON_UINT64_CHAIN_ID_IN_SPEC );
    chainid = d["cid"].GetUint64();

    ORACLE_CHECK_STATE3(
        d.HasMember( "uri" ), "No URI in Oracle spec:" + _spec, ORACLE_NO_URI_IN_SPEC );
    ...
    ORACLE_CHECK_STATE3(
        d.HasMember( "time" ), "No time pointer in Oracle spec:" + _spec, ORACLE_NO_TIME_IN_SPEC );
    ...
    ORACLE_CHECK_STATE3(
        d.HasMember( "pow" ), "No  pow in Oracle spec:" + _spec, ORACLE_NO_POW_IN_SPEC );
...
void OracleRequestSpec::parseWebRequestSpec( rapidjson::Document& d, const string& _spec ) {
    ...
    ORACLE_CHECK_STATE3(
        d["jsps"].IsArray(), "Jsps in Oracle spec is not array:" + _spec, ORACLE_JSPS_NOT_ARRAY );

    auto array = d["jsps"].GetArray();
    ORACLE_CHECK_STATE3( !array.Empty(), "Jsps array is empty.:" + _spec, ORACLE_JSPS_EMPTY );
    ORACLE_CHECK_STATE3( array.Size() <= ORACLE_MAX_JSPS, "Too many elements in JSP array:" + _spec,
        ORACLE_TOO_MANY_JSPS );

    for ( auto&& item : array ) {
        ORACLE_CHECK_STATE3(
            item.IsString(), "Jsps array item is not string:" + _spec, ORACLE_JSP_NOT_STRING );
        auto jsp = ( string ) item.GetString();
        ORACLE_CHECK_STATE3(
            jsp.size() <= ORACLE_MAX_JSP_SIZE, "JSP too long:" + _spec, ORACLE_JSP_TOO_LONG );
        jsps.push_back( jsp );
    }
    ...
    if ( d.HasMember( "trims" ) ) {
        ...
        ORACLE_CHECK_STATE3( jsps.size() == trims.size(),
            "hsps array size not equal tp trims array size:" + _spec,
...
void OracleRequestSpec::parseEthApiRequestSpec( rapidjson::Document& d, const string& _spec ) {
    ...
    ORACLE_CHECK_STATE3( params[0].IsObject(),
        "The first element in params array must be object " + _spec,
    ...
    ORACLE_CHECK_STATE3( params[0].MemberCount() == 4,
        ...
        ORACLE_PARAMS_ARRAY_INCORRECT_COUNT );
...
string OracleRequestSpec::checkAndGetParamsField(
    ...
    ORACLE_CHECK_STATE3( params[0][_fieldName.c_str()].IsString(),
        _fieldName + " field must be string " + _spec, ORACLE_INVALID_FIELD );
