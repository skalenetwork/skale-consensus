...
if [ "$UNIX_SYSTEM_NAME" = "Linux" ]; then
...
export INSTALL_ROOT_RELATIVE="$CUSTOM_BUILD_ROOT/../libBLS/deps/deps_inst/$ARCH"
...
if [ "$ARCH" = "x86_or_x64" ]; then
  ...
  if [ ! -d "$ARM_TOOLCHAIN_PATH" ]; then
    ...
    if [ ! -f $ARM_TOOLCHAIN_ARCH_NAME ]; then
      echo " "
      echo -e "${COLOR_SEPARATOR}=================================================${COLOR_RESET}"
    ...
    if [ ! -d "$ARM_TOOLCHAIN_PATH/arm-linux-gnueabihf/bin" ]; then
      echo " "
      echo -e "${COLOR_SEPARATOR}=================================================${COLOR_RESET}"
...
echo -e "${COLOR_VAR_NAME}WITH_BLAKE3${COLOR_DOTS}...........${COLOR_VAR_DESC}OpenSSL${COLOR_DOTS}................................${COLOR_VAR_VAL}$WITH_BLAKE3${COLOR_RESET}"
...
if [ "$WITH_BLAKE3" = "yes" ]; then
  ...
  if [ ! -f "$INSTALL_ROOT/lib/libblake3.a" ]; then
    ...
    cp libblake3.a ${INSTALL_ROOT}/lib
  else
    echo -e "${COLOR_SUCCESS}SKIPPED${COLOR_RESET}"
  fi
fi
...
if [ "$WITH_ZLIB" = "yes" ]; then
  ...
  if [ ! -f "$INSTALL_ROOT/lib/libz.a" ]; then
    ## (required for libssh)
    env_restore
    cd "$SOURCES_ROOT"
    if [ ! -d "zlib" ]; then
      if [ ! -f "zlib-from-git.tar.gz" ]; then
        echo -e "${COLOR_INFO}getting it from git${COLOR_DOTS}...${COLOR_RESET}"
...
if [ "$WITH_OPENSSL" = "yes" ]; then
  ...
  if [ ! -f "$INSTALL_ROOT/lib/libssl.a" ]; then
    ## openssl
    ...
    cd "$SOURCES_ROOT"
    if [ ! -d "openssl" ]; then
      if [ ! -f "openssl-from-git.tar.gz" ]; then
        echo -e "${COLOR_INFO}getting it from git${COLOR_DOTS}...${COLOR_RESET}"
...
if [ "$WITH_SSH" = "yes" ]; then
  ...
  if [ ! -d "$INSTALL_ROOT/include/libssh" ]; then
    ...
    cd "$SOURCES_ROOT"
    if [ ! -d "libssh" ]; then
      if [ ! -f "libssh-from-git.tar.gz" ]; then
        echo -e "${COLOR_INFO}getting it from git${COLOR_DOTS}...${COLOR_RESET}"
...
if [ "$WITH_CURL" = "yes" ]; then
  ...
  if [ ! -f "$INSTALL_ROOT/lib/libcurl.a" ]; then
    ...
    export PKG_CONFIG_PATH=/$INSTALL_ROOT/lib/pkgconfig:$PKG_CONFIG_PATH
    if [ ! -d "curl" ]; then
      if [ ! -f "curl-from-git.tar.gz" ]; then
        echo -e "${COLOR_INFO}getting it from git${COLOR_DOTS}...${COLOR_RESET}"
...
if [ ! -f "$INSTALL_ROOT/lib/libiconv.a" ]; then
  if [ "$UNIX_SYSTEM_NAME" = "Darwin" ]; then
...
if [ "$WITH_SDL" = "yes" ]; then
  ...
  if [ ! -f "$INSTALL_ROOT/lib/libSDL2.a" ]; then
    env_restore
    cd "$SOURCES_ROOT"
...
if [ "$WITH_SDL_TTF" = "yes" ]; then
  ...
  if [ ! -f "$INSTALL_ROOT/lib/libSDL2_ttf.a" ]; then
    env_restore
    cd "$SOURCES_ROOT"
    export PKG_CONFIG_PATH_SAVED=$PKG_CONFIG_PATH
    export PKG_CONFIG_PATH=/$INSTALL_ROOT/lib/pkgconfig:$PKG_CONFIG_PATH
...
if [ "$WITH_EV" = "yes" ]; then
  ...
  if [ ! -f "$INSTALL_ROOT/lib/libev.a" ]; then
    env_restore
    cd "$SOURCES_ROOT"
    if [ ! -d "libev" ]; then
      if [ ! -f "libev-from-git.tar.gz" ]; then
        echo -e "${COLOR_INFO}downloading it${COLOR_DOTS}...${COLOR_RESET}"
...
if [ "$WITH_EVENT" = "yes" ]; then
  echo -e "${COLOR_SEPARATOR}==================== ${COLOR_PROJECT_NAME}libEvent${COLOR_SEPARATOR} =====================================${COLOR_RESET}"
  if [ ! -f "$INSTALL_ROOT/lib/libevent.a" ]; then
    env_restore
    cd "$SOURCES_ROOT"
    if [ ! -d "libevent" ]; then
      if [ ! -f "libevent-from-git.tar.gz" ]; then
        echo -e "${COLOR_INFO}downloading it${COLOR_DOTS}...${COLOR_RESET}"
...
if [ "$WITH_UV" = "yes" ]; then
  ...
  if [ ! -f "$INSTALL_ROOT/lib/libuv.a" ]; then
    env_restore
    cd "$SOURCES_ROOT"
    if [ ! -d "libuv" ]; then
      if [ ! -f "libuv-from-git.tar.gz" ]; then
        echo -e "${COLOR_INFO}getting it from git${COLOR_DOTS}...${COLOR_RESET}"
...
if [ "$WITH_LWS" = "yes" ]; then
  echo -e "${COLOR_SEPARATOR}==================== ${COLOR_PROJECT_NAME}libWebSockets${COLOR_SEPARATOR} ================================${COLOR_RESET}"
  if [ ! -f "$INSTALL_ROOT/lib/libwebsockets.a" ]; then
    env_restore
    cd "$SOURCES_ROOT"
    if [ ! -d "libwebsockets" ]; then
      if [ ! -f "libwebsockets-from-git.tar.gz" ]; then
        echo -e "${COLOR_INFO}downloading it${COLOR_DOTS}...${COLOR_RESET}"
...
if [ "$WITH_SOURCEY" = "yes" ]; then
  echo -e "${COLOR_SEPARATOR}==================== ${COLOR_PROJECT_NAME}libSourcey${COLOR_SEPARATOR} ================================${COLOR_RESET}"
  if [ ! -f "$INSTALL_ROOT/lib/libscy_base.a" ]; then
    env_restore
    cd "$SOURCES_ROOT"
    if [ ! -d "libsourcey" ]; then
      echo -e "${COLOR_INFO}unpacking it${COLOR_DOTS}...${COLOR_RESET}"
...
if [ "$WITH_PUPNP" = "yes" ]; then
  # http://pupnp.sourceforge.net/
  echo -e "${COLOR_SEPARATOR}==================== ${COLOR_PROJECT_NAME}pupnp${COLOR_SEPARATOR} ========================================${COLOR_RESET}"
  if [ ! -f "$INSTALL_ROOT/lib/libupnp.a" ]; then
    env_restore
    cd "$SOURCES_ROOT"
    if [ ! -d "libupnp-1.8.3" ]; then
      echo -e "${COLOR_INFO}unpacking it${COLOR_DOTS}...${COLOR_RESET}"
...
if [ "$WITH_GNU_TLS" = "yes" ]; then
  echo -e "${COLOR_SEPARATOR}==================== ${COLOR_PROJECT_NAME}libGnuTLS${COLOR_SEPARATOR} ====================================${COLOR_RESET}"
  if [ ! -f "$INSTALL_ROOT/lib/libgnutls.a" ]; then
    env_restore
    cd "$SOURCES_ROOT"
    if [ ! -d "gnutls" ]; then
      echo -e "${COLOR_INFO}getting it from git${COLOR_DOTS}...${COLOR_RESET}"
...
if [ "$WITH_GPGERROR" = "yes" ]; then
  echo -e "${COLOR_SEPARATOR}==================== ${COLOR_PROJECT_NAME}liGpgError${COLOR_SEPARATOR} ===================================${COLOR_RESET}"
  if [ ! -f "$INSTALL_ROOT/lib/libgpg-error.a" ]; then
    env_restore
    cd "$SOURCES_ROOT"
    if [ ! -d "libgpg-error" ]; then
      echo -e "${COLOR_INFO}getting it from git${COLOR_DOTS}...${COLOR_RESET}"
...
if [ "$WITH_CRYPTOPP" = "yes" ]; then
  echo -e "${COLOR_SEPARATOR}==================== ${COLOR_PROJECT_NAME}libCrypto++${COLOR_SEPARATOR} ==================================${COLOR_RESET}"
  if [ ! -f "$INSTALL_ROOT/lib/libcryptopp.a" ]; then
    env_restore
    cd "$SOURCES_ROOT"

    if [ ! -d "libcryptopp" ]; then
      mkdir libcryptopp
...
if [ "$WITH_GMP" = "yes" ]; then
  ...
  if [ ! -f "$INSTALL_ROOT/lib/libgmp.a" ] || [ ! -f "$INSTALL_ROOT/lib/libgmpxx.a" ] || [ ! -f "$INSTALL_ROOT/lib/libgmp.la" ] || [ ! -f "$INSTALL_ROOT/lib/libgmpxx.la" ]; then
    env_restore
    cd "$SOURCES_ROOT"
    if [ ! -d "gmp-6.1.2" ]; then
...
if [ "$WITH_FF" = "yes" ]; then
  ...
  if [ ! -f "$INSTALL_ROOT/lib/libff.a" ]; then
    env_restore
    cd "$SOURCES_ROOT"
    if [ ! -d "libff" ]; then
      echo -e "${COLOR_INFO}getting it from git${COLOR_DOTS}...${COLOR_RESET}"
...
if [ "$WITH_PBC" = "yes" ]; then
  echo -e "${COLOR_SEPARATOR}==================== ${COLOR_PROJECT_NAME}PBC${COLOR_SEPARATOR} ==========================================${COLOR_RESET}"
  if [ ! -f "$INSTALL_ROOT/lib/libpbc.a" ] || [ ! -f "$INSTALL_ROOT/lib/libpbc.la" ]; then
    env_restore
    cd "$SOURCES_ROOT"
    if [ ! -d "pbc" ]; then
      echo -e "${COLOR_INFO}getting it from git${COLOR_DOTS}...${COLOR_RESET}"
...
if [ "$WITH_CURL" = "yes" ]; then
  ...
  if [ ! -f "$INSTALL_ROOT/lib/libzmq.a" ]; then
    # https://github.com/curl/curl
    env_restore
    cd "$SOURCES_ROOT"
    cd ../libzmq
