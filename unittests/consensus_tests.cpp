...
#ifdef BITE2
CATCH_TEST_CASE_METHOD( StartFromScratch,
    "Derive reencryption random from offchain signatures",
    "[consensus-reencryption-random][end-to-end]" ) {
    try {
        engine = new ConsensusEngine( 0, 1000000000 );
        engine->parseTestConfigsAndCreateAllNodes( Consensust::getConfigDirPath() );
        engine->slowStartBootStrapTest();
        ...
        for ( uint64_t blockId = 1; blockId <= ( uint64_t ) lastId; blockId++ ) {
            try {
                auto publicRandom = engine->getRandomForBlockId( blockId );
                auto reencryptionRandom1 = engine->getReencryptionRandomForBlockId( blockId );
                auto reencryptionRandom2 = engine->getReencryptionRandomForBlockId( blockId );

                // deterministic reads for the same block
                CATCH_REQUIRE( reencryptionRandom1 == reencryptionRandom2 );
                // offchain domain separation should produce a different random
                CATCH_REQUIRE( reencryptionRandom1 != publicRandom );
                checkedBlocks++;
            } catch ( ... ) {
                // Some very recent block IDs may not have offchain random persisted yet.
            }
        }
