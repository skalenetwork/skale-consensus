//
// Created by kladko on 8/22/22.
//

#ifndef SKALED_VECTOR_REF_H
#define SKALED_VECTOR_REF_H
...
public:
    ...
    vector_ref( typename std::conditional< std::is_const< _T >::value,
        std::vector< typename std::remove_const< _T >::type > const*, std::vector< _T >* >::type
            _data )
    ...
    std::string toString() const {
        return std::string(
            ( char const* ) m_data, ( ( char const* ) m_data ) + m_count * sizeof( _T ) );
    }
    ...
    explicit operator vector_ref< _T2 >() const {
        ...
        return vector_ref< _T2 >(
            reinterpret_cast< _T2* >( m_data ), m_count * sizeof( _T ) / sizeof( _T2 ) );
    }
