...
size_t RLP::length() const {
    ...
    if ( n < RLP_DATA_IMM_LEN_START )
        ...
        CHECK_STATE2(
            !( ret < RLP_LIST_START - RLP_DATA_IMM_LEN_START - RLP_MAX_LENGTH_BYTES ), "BadRLP" );
        ...
        if ( m_listStack.back().first )
            ...
            if ( s < RLP_LIST_IMM_LEN_COUNT )
                ...
                BOOST_THROW_EXCEPTION(
                    InvalidStateException( "RLP itemCount too large for RLP", __CLASS_NAME__ ) );
        }
...
RLPOutputStream& RLPOutputStream::append( bigint _i ) {
    if ( !_i )
        ...
        if ( br < RLP_DATA_IMM_LEN_COUNT )
            ...
            CHECK_STATE2( !( RLP_DATA_IND_LEN_ZERo + brbr > 0xff ),
                "RLP exception Number too large for RLP" );
...
static void streamOut( ostream& _out, RLP const& _d, unsigned _depth = 0 ) {
    if ( _depth > 64 )
        ...
        _out << showbase << hex << nouppercase << _d.toInt< bigint >( RLP::LaissezFaire ) << dec;
    else if ( _d.isData() )
