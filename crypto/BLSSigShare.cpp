...
BLSSigShare::BLSSigShare( ptr< string > _s, schain_id _schainID, block_id _blockID,
    schain_index _signerIndex, node_id _signerNodeID )
    : schainId( _schainID ),
      blockId( _blockID ),
    ...
    if ( _s->size() < 10 ) {
        BOOST_THROW_EXCEPTION( InvalidArgumentException(
            "Signature too short:" + to_string( _s->size() ), __CLASS_NAME__ ) );
    }

    if ( _s->size() > BLS_MAX_SIG_LEN ) {
        BOOST_THROW_EXCEPTION( InvalidArgumentException(
            "Signature too long:" + to_string( _s->size() ), __CLASS_NAME__ ) );
    }
    ...
    if ( position == string::npos ) {
        BOOST_THROW_EXCEPTION(
            InvalidArgumentException( "Misformatted sig:" + *_s, __CLASS_NAME__ ) );
    }
...
BLSSigShare::BLSSigShare( ptr< libff::alt_bn128_G1 >& _s, schain_id _schainId, block_id _blockID,
    schain_index _signerIndex, node_id _nodeID )
    : schainId( _schainId ),
      blockId( _blockID ),
      signerIndex( _signerIndex ),
      signerNodeId( _nodeID ) {
    if ( signerIndex == 0 ) {
        BOOST_THROW_EXCEPTION( InvalidArgumentException( "Zero signer index", __CLASS_NAME__ ) );
    }
