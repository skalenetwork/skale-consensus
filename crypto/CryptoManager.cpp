...
bool CryptoManager::signECDSASigRSOpenSSL(
    string& pubKeyStr, const char* hash) {
    ...
    else
    {
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    ECDSA_SIG *signature = ECDSA_do_sign((const unsigned char*) hash, 32, eckey);
                    if (NULL == signature)
                    {
                    ...
                    else
                    {
                        ...
                        int verify_status = ECDSA_do_verify((const unsigned char*) hash, 32, signature, eckey);
...
bool CryptoManager::verifyECDSASigRSOpenSSL(
    ...
    CHECK_STATE(pubKeyStr.size() == 128)
    CHECK_ARGUMENT(hashHex);CHECK_ARGUMENT(signatureR);
    ...
    else
    {
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    ECDSA_SIG *signature = ECDSA_do_sign((const unsigned char*) hashHex, 32, eckey);
                    if (NULL == signature)
                    {
                        printf("Failed to generate EC Signature\n");
                        function_status = -1;
                    ...
                    else
                    {
                        ...
                        int verify_status = ECDSA_do_verify((const unsigned char*) hashHex, 32, signature, eckey);
                        const int verify_success = 1;
                        if (verify_success != verify_status)
                        {
                            printf("Failed to verify EC Signature\n");
