...
CryptoManager::CryptoManager(Schain &_sChain)
    ...
    if (isSGXEnabled) {
        ...
        // verify that the SGX ecdsa public key of the current node on SGX server equal to the key
        // in config
        auto ecdsaPublicKeyOnServer = getSGXEcdsaPublicKey(sgxECDSAKeyName, getSgxClient());
        auto ecdsaPublicKeyInConfig =
                ecdsaPublicKeyMap.at((uint64_t) getSchain()->getNode()->getNodeID());
        ...
        if (isSGXEnabled) {
            ...
            try {
                ...
                throw_with_nested(InvalidStateException(
                    __FUNCTION__, __CLASS_NAME__ + string(" Could not verify EdDSA sig")));
            }
...
ptr<ThresholdSigShare> CryptoManager::signDAProofSigShare(
    ...
    if (getSchain()->getNode()->isSgxEnabled() && !_forceMockup) {
        ...
        return make_shared<ConsensusEdDSASigShare>(
            share, sChain->getSchainID(), _blockId, _timestamp, totalSigners);
    } else {
...
void CryptoManager::verifyDAProofSigShare(ptr<ThresholdSigShare> _sigShare,
                                          schain_index _schainIndex, BLAKE3Hash &_hash, node_id, bool _forceMockup) {
    MONITOR(__CLASS_NAME__, __FUNCTION__)
...
ptr<vector<ptr<AESKeyDecryptionShare> > > CryptoManager::sgxDecryptAESKeyShareBatch(
    ...
    CHECK_STATE(getSchain()->getNode()->isSgxEnabled())
...
ptr<ThresholdSigShareSet> CryptoManager::createDAProofSigShareSet(
    block_id _blockId, uint64_t _timestamp) {
    if (getSchain()->getNode()->isSgxEnabled()) {
...
void CryptoManager::verifySessionSigAndKey(BLAKE3Hash &_hash, const string &_sig,
                                           ...
                                           pair<node_id, node_id> _nodeId, uint64_t _timeStamp) {
    MONITOR(__CLASS_NAME__, __FUNCTION__);
...
ptr<ThresholdSignature> CryptoManager::verifyDAProofThresholdSig(
    ...
    try {
        if (verifyRealSignatures) {
            auto sig = make_shared<ConsensusEdDSASignature>(_signature,
                                                            getSchain()->getSchainID(), _blockId, _timestamp,
            ...
            if (!this->isSyncNode) {
                CHECK_STATE2(sig->toString() == _hash.toHex(),
                             ...
                             to_string( this->isSyncNode ) +
                             ":VRS:" + to_string( verifyRealSignatures ));
            }
